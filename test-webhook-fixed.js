...
const commands = {
  ...
  'esp32-status': async () => {
    console.log('📱 Simulando consulta do ESP32...');
    try {
      const response = await axios.get(`${BASE_URL}/coffee-status`, {
        headers: {
          'User-Agent': 'ESP32-Arduino/1.0 (ESP32)'
        }
      });
      console.log('✅ Status ESP32:', response.data);
    } catch (error) {
      console.error('❌ Erro:', error.response?.data || error.message);
    }
  },
...
async function main() {
  ...
  if (!command || !commands[command]) {
    ...
    console.log('  esp32-status        - Simular consulta do ESP32');
